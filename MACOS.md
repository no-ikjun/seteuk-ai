...
```bash
shasum -a 256 "Seteuk AI_<버전>_aarch64.dmg"
```
...
1. 다운로드한 `Seteuk AI_<버전>_aarch64.dmg`를 엽니다.
2. `Seteuk AI.app`을 `Applications` 폴더로 드래그합니다.
3. 응용 프로그램 폴더에서 `Seteuk AI`를 실행합니다.
...
종료한 뒤 새 `Seteuk AI.app`을 Applications 폴더의 기존 앱 위에 복사하면
됩니다.
...
응용 프로그램 폴더의 `Seteuk AI.app`을 휴지통으로 이동합니다. 앱은 별도의
